...
export default function EditProject() {
  ...
  const isCompleted = statuses.find(s => s.id.toString() === formData.status_id.toString())?.status === 'Completed';
  ...
  async function fetchProject() {
    try {
      const { data, error } = await supabase
        .from('projects')
        .select('*, project_scope(id, scope), category(id, category), projects_image(path), projects_video(url)')
        .eq('id', id)
        .single();
      ...
      if (data) {
        setFormData({
          ...
          video_url: (data as any).projects_video?.[0]?.url || "",
          started_at: data.started_at || "",
          end_at: data.end_at || "",
        });

        if (data.cover_image) {
          setPreviews(prev => ({ ...prev, cover: data.cover_image }));
        }

        const galleryPaths = (data as any).projects_image?.map((img: any) => img.path) || [];
        const newGalleryPreviews = ["", "", "", "", "", ""];
        galleryPaths.forEach((path: string, idx: number) => {
          if (idx < 6) newGalleryPreviews[idx] = path;
        });
        setPreviews(prev => ({ ...prev, gallery: newGalleryPreviews }));
      }
    } catch (error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (let i = 0; i < galleryFiles.length; i++) {
        ...
        } else if (previews.gallery[i]) {
          galleryUrls.push({ path: previews.gallery[i] });
        }
      }
  ...
  if (fetching) return <div className="p-20 text-center font-bold uppercase tracking-widest text-slate-400">Loading...</div>;

  return (
    <div className="space-y-10 pb-20">
      <ScrollReveal>
        <div className="flex items-center gap-4">
          <Link href="/admin/projects" className="p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl text-slate-400 hover:text-emerald-600 hover:border-emerald-500/30 transition-all shadow-sm">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="w-8 h-[2px] bg-emerald-600"></div>
              <span className="text-emerald-600 font-bold uppercase tracking-[0.4em] text-[9px]">Portfolio Refinement</span>
            </div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white uppercase tracking-tighter">Edit <span className="text-emerald-600">Project.</span></h1>
          </div>
        </div>
      </ScrollReveal>
              ...
              <div className="relative z-10 space-y-8">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Type className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-600 transition-colors" size={18} />
                    <input required type="text" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full pl-14 pr-6 py-4 bg-white dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-2xl outline-none focus:border-emerald-500 transition-all font-medium shadow-sm text-slate-900 dark:text-white placeholder:text-slate-400" />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className={`w-full px-6 py-4 border rounded-2xl outline-none transition-all font-medium shadow-sm ${!isCompleted ? 'bg-slate-100 dark:bg-slate-900/50 border-gray-100 dark:border-slate-800 cursor-not-allowed opacity-50' : 'bg-white dark:bg-slate-950 border-gray-200 dark:border-slate-800 focus:border-emerald-500 text-slate-900 dark:text-white'}`} 
                    />
                  </div>
                </div>
